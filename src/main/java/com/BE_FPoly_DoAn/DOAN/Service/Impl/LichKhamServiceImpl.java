...
public class LichKhamServiceImpl {
    ...
    public ServiceResponse<?> create(LichKhamDTO dto) {
        ...
        if (dto.getThoiGianHen() == null || dto.getThoiGianDen() == null || dto.getNgayKham() == null) {
            return ServiceResponse.error(NotificationCode.APPOINTMENT_INVALID_TIME, "Thời gian khám không được để trống");
        }

        if (!dto.getThoiGianHen().isBefore(dto.getThoiGianDen())) {
            return ServiceResponse.error(NotificationCode.APPOINTMENT_INVALID_TIME, "Thời gian hẹn phải trước thời gian đến");
        }

        if (dto.getNgayKham().isBefore(LocalDate.now())) {
            return ServiceResponse.error(NotificationCode.APPOINTMENT_INVALID_TIME, "Không thể đặt lịch cho ngày trong quá khứ");
        }
        ...
        int soLich = lichKhamRepo.countLichKhamByBacSiAndNgayAndCa(Long.valueOf(dto.getBacSiId()), dto.getNgayKham(), dto.getCaKham());
        int gioiHanLich = 5;
        if (soLich >= gioiHanLich) {
            return ServiceResponse.error("APP_014", "Bác sĩ đã đủ lịch trong ca " + dto.getCaKham());
        }
        ...
        if (!isThoiGianHopLeTheoCa(thoiGianHen, caKham)) {
            return ServiceResponse.error(NotificationCode.APPOINTMENT_INVALID_TIME, "Giờ hẹn không phù hợp với ca khám đã chọn: " + caKham);
        }
        ...
        try {
            ...
            gioKhamChiTietList.stream()
                    .filter(slot -> !slot.getThoiGian().isBefore(from) && !slot.getThoiGian().isAfter(to))
                    .forEach(slot -> {
                        slot.setTrangThai(false);
                    });
    ...
    public ServiceResponse<?> update(Integer id, LichKhamDTO dto) {
        ...
        if (dto.getNgayKham().isBefore(LocalDate.now())) {
            return ServiceResponse.error(NotificationCode.APPOINTMENT_INVALID_TIME, "Không thể cập nhật lịch cho ngày đã qua");
        }
